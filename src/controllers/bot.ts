...
export class BotController {
  ...
  private setupCommands(): void {
    ...
    this.bot.command('change_language', (ctx) => this.handleChangeLanguage(ctx));
  }
  ...
  private setupCallbackQueries(): void {
    ...
    this.bot.action('output_english', (ctx) => this.handleOutputEnglish(ctx));
  ...
  private async handleStart(ctx: Context): Promise<void> {
    ...
    this.userStateService.setUserSettings(userId, { input: 'hindi', output: 'hindi' });
  ...
  private async handleChangeLanguage(ctx: Context): Promise<void> {
    const keyboard = {
      inline_keyboard: [
        [{ text: '🎤 Choose Input Language', callback_data: 'select_input' }],
        [{ text: '🔊 Choose Output Language', callback_data: 'select_output' }],
        [{ text: '📋 View Current Settings', callback_data: 'view_settings' }],
      ],
    };
  ...
  private async handleSelectInput(ctx: Context): Promise<void> {
    ...
    await ctx.editMessageText('🎤 Choose your input language:', { reply_markup: keyboard });
  }
  ...
  private async handleSelectOutput(ctx: Context): Promise<void> {
    ...
    await ctx.editMessageText('🔊 Choose your output language:', { reply_markup: keyboard });
  }
  ...
  private async handleViewSettings(ctx: Context): Promise<void> {
    ...
    const { inputFlag, outputFlag } = this.userStateService.getLanguageFlags(settings);
  ...
  private async handleBackToMain(ctx: Context): Promise<void> {
    ...
    await ctx.editMessageText('🌐 Language Settings:', { reply_markup: keyboard });
  }
  ...
  private async handleVoiceMessage(ctx: Context): Promise<void> {
    ...
    try {
      const result = await this.processorService.processVoiceMessage(userId, fileLink.href);
  ...
  private async processUserInput(
    ...
    const thinkingMessage = await ctx.reply('Thinking... (' + inputIcon + inputFlag + '→' + outputFlag + ')');

    try {
      const result = await this.processorService.processTextInput(userId, inputText);
  ...
  private async sendAudioResponse(
    ...
    const oggPath = await this.processorService.generateAudioFile(result.audioBase64);

    try {
      const caption = inputIcon + inputFlag + ' Input: "' + result.inputText + '"\n🔊' + outputFlag + ' Output: ' + result.outputText;
