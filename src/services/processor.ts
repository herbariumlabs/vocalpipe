...
import { convertOgaToWav, convertWavToOgg, saveBase64ToWav, readFileAsBase64 } from '../utils/audio';
import { generateUniqueId, generateFilePaths, cleanupFiles } from '../utils/file';
...
export class ProcessorService {
  ...
  async processVoiceMessage(
    ...
    try {
      ...
      console.log(`🗣 ${settings.input.toUpperCase()} STT:`, recognizedText);
  ...
  async processTextInput(
    ...
    const gptReply = await this.openaiService.generateResponse(englishPrompt);
    ...
    if (settings.output === 'hindi') {
      ...
      audioBase64 = await this.bhashiniService.textToSpeech(finalText);
    } else {
      finalText = gptReply;
